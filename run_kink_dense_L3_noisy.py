"""
Sweep B: L=3 noisy dense kink sweep (p=0.01, eps=SPAM_DEFAULT), companion to
run_kink_dense_L3_noiseless.py. Redone at L=3 after
results/boundary_restart_scaling_N3.csv established L=2 cannot represent the
ground state near K=4-5.25; the original L=2 noisy sweep
(kink_dense_N3.csv) is invalid as a noise-attributable baseline in this
window because its own noiseless counterpart was already 65% wrong from
expressibility, not noise.

N=3, L=3, GI, p=0.01, eps=SPAM_DEFAULT (0.005), K=linspace(4,7,13),
8 restarts, maxiter=2000. n_CX = 2*L*(nq-1) = 30 (NOT NCX_N3_L2=20, the L=2
...
N_RESTARTS = 8
MAXITER = 2000
SEED_BASE = 100000  # distinct from 10000/20000/.../90000
N_WORKERS = 13
N_CX = 2 * L * (N * F - 1)  # 30, not NCX_N3_L2 (=20, the L=2 value)

RESULTS_DIR = "results"
CSV_PATH = os.path.join(RESULTS_DIR, "kink_dense_N3_L3_noisy.csv")
CKPT_PATH = os.path.join(RESULTS_DIR, "kink_dense_N3_L3_noisy_checkpoint.json")
...
def main():
    ...
    w_eff = (1 - P) ** N_CX * (1 - 2 * EPS)
    print(f"L={L}, n_CX={N_CX} (not NCX_N3_L2={core.NCX_N3_L2}, the L=2 value), "
          f"cumulative infidelity 1-(1-p)^n_CX = {(1-(1-P)**N_CX)*100:.1f}%, w_eff={w_eff:.4f}", flush=True)
    ...
    print(f"Dense kink sweep L=3 (noisy p={P}): {total_cells} K cells total, {len(done_cells)} already done, "
    ...
    print(f"Dense kink sweep L=3 (noisy) done in {(time.time()-t_start)/60:.1f} min.")
