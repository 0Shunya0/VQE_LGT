"""
Sweep A: L=3 noiseless baseline for the dense kink sweep, redone after
results/boundary_restart_scaling_N3.csv established that L=2 cannot
represent the ground state near K=4-5.25 (65.2% error at K=4.5 even with 32
restarts/maxiter=4000) while L=3 recovers it (0.08% error, 8 restarts). The
original L=2 dense-sweep baseline (kink_dense_N3_noiseless.csv) is invalid
in this window and is superseded by this sweep.

N=3, L=3, GI, p=0, eps=0, K=linspace(4,7,13), 8 restarts, maxiter=2000.
...
N_RESTARTS = 8
MAXITER = 2000
SEED_BASE = 90000  # distinct from 10000/20000/30000/40000/50000/60000/70000/80000
N_WORKERS = 13
N_CX = 2 * L * (N * F - 1)  # 30, not NCX_N3_L2 (=20, the L=2 value)

RESULTS_DIR = "results"
CSV_PATH = os.path.join(RESULTS_DIR, "kink_dense_N3_L3_noiseless.csv")
CKPT_PATH = os.path.join(RESULTS_DIR, "kink_dense_N3_L3_noiseless_checkpoint.json")
...
def main():
    ...
    print(f"L={L}, n_CX={N_CX} (not NCX_N3_L2={core.NCX_N3_L2}, the L=2 value)", flush=True)
    ...
    print(f"Dense kink sweep L=3 (noiseless): {total_cells} K cells total, {len(done_cells)} already done, "
    ...
    print(f"Dense kink sweep L=3 (noiseless) done in {(time.time()-t_start)/60:.1f} min.")
