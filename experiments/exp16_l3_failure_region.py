"""
EXPERIMENT 16: does L=3 fix the L=2 representation failure across the WHOLE
region where it occurs (K=1..5, per exp15), or only at the single point
(K=4.5) where L=3 has ever actually been checked?

Modelled on exp15_l2_full_sweep_noiseless.py / exp10, just L=3 and the five
K points spanning exp15's failure region instead of the full grid.
"""
...
N_RESTARTS = 8
MAXITER = 2000
SEED_BASE = 180000  # distinct from every prior sweep (10000..170000)
N_WORKERS = 5

RESULTS_DIR = "results"
CSV_PATH = os.path.join(RESULTS_DIR, "l3_failure_region_N3.csv")
CKPT_PATH = os.path.join(RESULTS_DIR, "l3_failure_region_N3_checkpoint.json")
...
def main():
    ...
    print(f"L=3 failure-region check: {total_cells} K cells total, {len(done_cells)} already done, "
    ...
    print(f"L=3 failure-region check done in {(time.time()-t_start)/60:.1f} min.")
